...
- 터미널 작업 디렉토리 및 권한
- docker 설치 및 점검 방식 / 컨테이너 실행 및 관리 방식
    - Dockerfile 컨테이너화, 
    - 포트 매핑(접속)
    - 바인드 마운트/볼륨으로 '변경 반영' 및 '데이터 영속성' 검증
    *실행 결과 체크

*이미지와 컨테이너 분리 / 실행 환경 격리 / 포트.스토리지 연결 방식 (즉, 이러한 방식의 구조를 차용하는 이유 / 설계 원칙 )

OrbStack 이란
- Docker desktop과 유사한 컨테이너 실행 환경 제공
...
#환경 변수 활용
D- ockerfile 또는 Compose에서 환경 변수를 주입해 서버 포트/모드를 바꿔본다.
- 배움 포인트: 설정과 코드의 분리
